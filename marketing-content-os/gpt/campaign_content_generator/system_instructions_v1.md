...
Unsafe optional overrides: reject only the unsafe override and continue safe rows when required inputs and SKU are valid. Examples: unsupported promo, official endorsement, official affiliation, guarantee, real competition/exam claim. State rejected override briefly outside TSV; never put rejection/policy language in customer copy. Stop with zero rows only for invalid SKU, missing required input, approved truth conflict, unknown forced template, or impossible/unresolvable required request.

...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.setStyleSheet(
            ...
            QLabel#MetricLabel, QLabel#PanelHelp { font-size: 12px; color: #5f6f86; }
        ...
        self.step_badges = [
            StepBadge("1", "เลือกสินค้า + ตั้ง N"),
            StepBadge("2", "Copy GPT1 prompt"),
            StepBadge("3", "เลือก/สร้าง raw folder"),
        ...
        self.copy_gpt1_button = self.action_button("1. Copy GPT1 Prompt", self.copy_gpt1_prompt, "SuccessButton")
        self.choose_folder_button = self.action_button("2. Choose Raw Folder", self.choose_folder, "CommandButton")
        self.workspace_button = self.action_button("Create/Open SKU Raw Folder", self.create_or_open_sku_workspace, "CommandButton")
        self.clean_button = self.action_button("3. Run Pipeline", self.run_cleaner, "PrimaryButton")
        self.clean_button.setEnabled(False)
        self.open_output_button = self.action_button("Open _cleaned Folder", self.open_output_folder, "CommandButton")
        self.open_output_button.setEnabled(False)
        self.reset_button = self.action_button("Reset", self.reset_workflow, "DangerButton")

        self.command_copy_gpt1_button = self.action_button("1. Copy GPT1 Prompt", self.copy_gpt1_prompt, "SuccessButton")
        self.command_workspace_button = self.action_button("Create/Open SKU Raw Folder", self.create_or_open_sku_workspace, "CommandButton")
        self.command_choose_folder_button = self.action_button("2. Choose Raw Folder", self.choose_folder, "CommandButton")
        self.command_clean_button = self.action_button("3. Run Pipeline", self.run_cleaner, "PrimaryButton")
        self.command_clean_button.setEnabled(False)
        self.command_open_output_button = self.action_button("Open _cleaned Folder", self.open_output_folder, "CommandButton")
        self.command_open_output_button.setEnabled(False)
        self.command_reset_button = self.action_button("Reset", self.reset_workflow, "DangerButton")
        ...
        self.copy_prompt_button = self.action_button("Copy First GPT2 Prompt", self.copy_first_prompt, "SuccessButton")
        self.open_selected_button = self.action_button("Open Selected N Rows", self.open_selected_file)
        self.open_prompts_button = self.action_button("Open GPT2 Prompts Folder", self.open_prompt_folder)
        self.open_clean_button = self.action_button("Open Clean TSV", self.open_clean_file)
        self.open_report_button = self.action_button("Open Report", self.open_report_file)
    ...
    def build_gpt1_tab(self) -> QWidget:
        ...
        help_text = QLabel("เลือกสินค้าจากชื่อที่อ่านง่าย โปรแกรมจะใช้ SKU ที่ถูกต้องเป็น value ใน GPT1 prompt")
        ...
        form.addWidget(QLabel("Product details"), 1, 0)
        ...
        n_row.addWidget(QLabel("Presets:"))
        for button in self.preset_buttons:
            n_row.addWidget(button)
        n_row.addStretch(1)
        form.addLayout(n_row, 2, 1, 1, 4)
        form.addWidget(QLabel("GPT1 prompt"), 3, 0)
        form.addWidget(self.gpt1_prompt_preview, 3, 1, 1, 4)
        form.addWidget(self.copy_gpt1_button, 4, 1, 1, 4)
        layout.addLayout(form)
        next_steps = QLabel("หลัง GPT1 ตอบ ให้ save output ทั้งหมดเป็นไฟล์ .md หรือ .txt ใน raw folder ของ SKU แล้วไปแท็บ 2. Import & Clean")
        next_steps.setObjectName("PanelHelp")
    ...
    def build_import_tab(self) -> QWidget:
        ...
        panel.setObjectName("Panel")
        layout = QVBoxLayout(panel)
        title = QLabel("2. Import GPT1 raw output and run pipeline")
        title.setObjectName("PanelTitle")
        help_text = QLabel("ใช้ Create/Open SKU Raw Folder เพื่อลดการจำ path หรือเลือก folder เองก็ได้")
        help_text.setObjectName("PanelHelp")
        help_text.setWordWrap(True)
        layout.addWidget(title)
        layout.addWidget(help_text)
        form = QGridLayout()
        form.addWidget(QLabel("Raw folder"), 0, 0)
    ...
    def build_handoff_tab(self) -> QWidget:
        ...
        panel.setObjectName("Panel")
        layout = QVBoxLayout(panel)
        title = QLabel("4. GPT2 handoff")
        title.setObjectName("PanelTitle")
        help_text = QLabel("เมื่อมีไฟล์ PASS แล้ว ใช้ปุ่มด้านล่างเพื่อ copy prompt หรือเปิดโฟลเดอร์ handoff")
        help_text.setObjectName("PanelHelp")
        help_text.setWordWrap(True)
        layout.addWidget(title)
        layout.addWidget(help_text)
    ...
    def build_review_tab(self) -> QWidget:
        ...
        panel.setObjectName("Panel")
        layout = QVBoxLayout(panel)
        title = QLabel("5. Image generation + human review")
        title.setObjectName("PanelTitle")
        layout.addWidget(title)
        text = QTextEdit()
    ...
    def on_product_changed(self) -> None:
        ...
        self.product_card.setPlainText(
            ...
            f"Claim class: {product.get('CLAIM_POLICY_CLASS', '')}"
        )
        self.update_gpt1_prompt_preview()
        self.set_coach(1, "เลือกสินค้าแล้ว", "ตรวจ Product details แล้วกด Copy GPT1 Prompt")
    ...
    def set_coach(self, stage: int, title: str, text: str) -> None:
        ...
        next_map = {
            1: "Next: ตรวจสินค้าและจำนวน N แล้วกด Copy GPT1 Prompt",
            2: "Next: วาง prompt ใน GPT1 แล้ว save output เป็น .md หรือ .txt",
            3: "Next: วาง raw output ใน folder แล้วกด Run Pipeline",
            4: "Next: รอผล PASS / FAIL จาก deterministic validation",
            5: "Next: Copy GPT2 Prompt สำหรับไฟล์ PASS แล้วทำ GPT2 ต่อ",
        }
    ...
    def create_or_open_sku_workspace(self) -> None:
        ...
        self.raw_count.setText(str(len(discover_raw_files(folder))))
        self.set_action_buttons(False)
        self.set_coach(3, "เปิด raw folder ของ SKU แล้ว", "Save GPT1 output เป็น .md หรือ .txt ใน folder นี้ จากนั้นกด Run Pipeline")
    ...
    def choose_folder(self) -> None:
        ...
        if has_raw:
            self.set_coach(3, f"พบไฟล์ raw {len(raw_files)} ไฟล์", f"กด Run Pipeline เพื่อ validate expected rows = {self.get_post_count()} และสร้าง GPT2 prompts")
        else:
            self.set_coach(3, "ยังไม่พบไฟล์ raw", "นำ output จาก GPT1 ไป save เป็น .md หรือ .txt ในโฟลเดอร์นี้ แล้วเลือก/เปิดโฟลเดอร์อีกครั้ง")

    def reset_results_only(self) -> None:
        self.results = []
        self.summary = None
        self.table.setRowCount(0)
        self.preview.setPlainText("เลือกโฟลเดอร์แล้ว กด Run Pipeline เพื่อเริ่มตรวจและเตรียมไฟล์ส่งต่อ")
    ...
    def run_cleaner(self) -> None:
        if self.selected_folder is None:
            QMessageBox.warning(self, "Missing folder", "Choose or create a raw folder first.")
            return
        self.normalize_n_from_input()
        self.clean_button.setEnabled(False)
        self.command_clean_button.setEnabled(False)
        ...
        self.set_action_buttons(False)
        self.set_coach(4, "กำลัง run pipeline", f"โปรแกรมกำลังเตรียม clean TSV และ N={self.get_post_count()} GPT2 prompts ต่อไฟล์ที่ PASS")
    ...
    def on_clean_finished(self, summary: PipelineBatchSummary) -> None:
        ...
        if first_pass_index is not None:
            ...
            self.set_coach(5, "เตรียม GPT2 prompts เสร็จแล้ว", "เปิดแท็บ GPT2 Handoff แล้วใช้ prompt ต่อได้")
        elif self.results:
            self.set_action_buttons(False)
            self.open_output_button.setEnabled(True)
            self.command_open_output_button.setEnabled(True)
    ...
    def reset_workflow(self) -> None:
        ...
        self.table.setRowCount(0)
        self.results_empty.setText("ยังไม่มีผลลัพธ์: Copy GPT1 Prompt → save output เป็น .md/.txt → Create/Open Raw Folder → Run Pipeline")
        self.results_empty.setVisible(True)
        self.preview.setPlainText("พื้นที่นี้จะแสดงผลหลังไฟล์ PASS:\n- selected N rows\n- GPT2 prompt แรก\n- คำแนะนำขั้นถัดไป")
        ...
        self.command_open_output_button.setEnabled(False)
        self.set_action_buttons(False)
        self.set_coach(1, "เลือกสินค้าจากรายการ ไม่ต้องจำ SKU", "โปรแกรมใช้ชื่อสินค้าที่อ่านง่าย แต่ value ที่ส่งให้ GPT1 คือ SKU ที่ถูกต้อง")

...
class MainWindow(QMainWindow):
    ...
    def qss(self) -> str:
        chevron = self.paths.qss_icon_url("chevron-down.svg")
        return f"""
            ...
            QComboBox::drop-down {{ border-left: 1px solid #94a3b8; width: 34px; background: #e7edf7; border-top-right-radius: 8px; border-bottom-right-radius: 8px; }}
            ...
            QTabWidget::pane {{ border: 1px solid #dbe3ef; background: #ffffff; border-radius: 8px; }}
            QTabBar::tab {{ background: #e7edf7; color: #10203a; padding: 10px 16px; border: 1px solid #cbd6e7; border-top-left-radius: 8px; border-top-right-radius: 8px; margin-right: 2px; }}
            ...
            QScrollArea {{ border: 0; background: #f6f8fb; }}
        """
    ...
    def build_ui(self) -> QWidget:
        ...
        self.tabs.addTab(self.scrollable(self.build_gpt1_panel()), "1. สร้างคำสั่ง GPT1")
        self.tabs.addTab(self.scrollable(self.build_check_tab()), "2. ตรวจคำตอบ GPT1")
        self.tabs.addTab(self.scrollable(self.build_prompt_panel()), "3. ส่งเข้า GPT2")
    ...
    def build_gpt1_panel(self) -> QFrame:
        ...
        count_layout.addWidget(QLabel("จำนวนโพสต์ต่อสินค้า:"))
        self.minus_button = self.icon_button("minus.svg", "ลดจำนวน", lambda: self.adjust_n(-1))
        ...
        self.plus_button = self.icon_button("plus.svg", "เพิ่มจำนวน", lambda: self.adjust_n(1))
        count_layout.addWidget(self.minus_button)
        count_layout.addWidget(self.n_value)
        count_layout.addWidget(self.plus_button)
        count_layout.addWidget(QLabel("เลือกเร็ว:"))
        ...
        layout.addLayout(action_row, 7, 0, 1, 3)
    ...
    def build_metrics(self) -> QFrame:
        ...
        layout = QHBoxLayout(panel)
        layout.setContentsMargins(10, 8, 10, 8)
        self.job_metric = self.make_metric_card("0", "สินค้า")
        self.pass_metric = self.make_metric_card("0", "ผ่าน")
    ...
    def make_metric_card(self, value: str, label: str) -> QFrame:
        frame = QFrame()
    ...
    def build_prompt_panel(self) -> QWidget:
        panel = QWidget()
        layout = QGridLayout(panel)
        title = QLabel("ส่งเข้า GPT2")
        title.setObjectName("PanelTitle")
        help_text = QLabel(f"เลือกสินค้าที่ผ่านจากแท็บตรวจคำตอบ GPT1 แล้วใช้ไฟล์ใน {READY_DIR_NAME} เพื่อส่งเข้า GPT2 ทีละรายการ")
        help_text.setObjectName("Help")
        help_text.setWordWrap(True)
    ...
    def build_summary_tab(self) -> QWidget:
        ...
        title = QLabel("สรุปผล")
        title.setObjectName("PanelTitle")
        help_text = QLabel("หลังตรวจเสร็จ ใช้ปุ่มเปิดสรุปรวมหรือเลือกสินค้าที่ผ่านเพื่อเปิดโฟลเดอร์พร้อมส่ง GPT2")
        help_text.setObjectName("Help")
        help_text.setWordWrap(True)
        layout.addWidget(title)
        layout.addWidget(help_text)
        ...
        self.summary_preview.setPlainText("ยังไม่มีผลสรุป")
        layout.addWidget(self.summary_preview, stretch=1)
        return panel
    ...
    def on_finished(self, summary: ParallelWorkspaceSummary) -> None:
        ...
        if summary.pass_job_count > 0:
            self.next_action.setText("ขั้นต่อไป: ไปแท็บ 3 เลือกสินค้าที่ผ่าน แล้วคัดลอกคำสั่ง GPT2 ทีละรายการ")
